...
## feat: the Render image button starts ComfyUI itself (2026-09-07)

**Problem:** The render path shipped with one deliberate hole: nothing in the app could start ComfyUI, so with the engine down a click on "Render image" ended in a card that said "Start it with: bash engine/comfy-headless.sh". That is the correct answer for the SCHEDULED path, where a 09:13 cron job on a machine that may be asleep must never decide on its own to load a 17 GB checkpoint (measured: it expands to roughly 24 GB resident and took this machine's swap from 12 GB to 36.4 GB in use). It is the wrong answer for a deliberate click, because clicking the button IS that decision, and handing a person a shell command at the exact moment they asked for an image is the friction the button existed to remove.

**Solution:** `POST /api/render` probes the engine before doing anything and, when it does not answer, runs `bash engine/comfy-headless.sh` and waits for it (the script was already idempotent, polls `/system_stats`, and exits 0 only once the server answers, 1 on failure, 180 s cap) before spawning the renderer. A failed start writes a result whose error carries the script's own diagnosis and stops there: no render is attempted against an engine that is not there. The diagnosis is the FIRST `shippost:`-prefixed stderr line, not the last one, because the likeliest failure prints the cause first and a generic "Install ComfyUI or set SHIPPOST_COMFY_DIR" hint second, so the renderer's last-line convention would have shown the hint and hidden the path; and the log is named only when it exists, since that same failure exits before the script ever creates it. A start is refused outright when the configured base URL is not on this machine: `comfy-headless.sh` always launches `main.py --listen 127.0.0.1` and only POLLS the configured URL, so starting on behalf of a remote engine would boot a local 20 GB server nobody asked for, wait out the full 180 s against a host that was never going to answer, and then report failure. The route also resolves the base URL itself, honouring `SHIPPOST_COMFY_PORT` the way the shell script does and passing the result to the renderer as `SHIPPOST_COMFY_URL`, which was previously the one env var that made the three call sites disagree.

So the card is not a two-minute dead spinner, the GET gained a `phase` (`starting`, then `rendering`, then null) published through a `.render_phase` file next to the existing lock and result files, read only while the lock says a run is live so a killed run cannot leave the card claiming "starting" forever. The shape stays backward compatible: `running` and `lastResult` are untouched. The button reads "Starting the image engine…" with a hint line under it during the start, and the client guards against a null phase, which is the legitimate answer whenever `generate.sh` holds the lock and writes no phase at all.

The whole run, start plus render, stays under the ONE `.rendering` lock, released on every path including a failed start. Because 180 s of start plus a 12 minute render plus the renderer's queue and download timeouts totals ~17.5 min against a 15 min `STALE_MS`, the lock is now heartbeated (`touchRunLock`, compare-and-utime, once a minute) rather than the render budget being cut: lowering the timeout does not actually fix it (even 10 minutes still totals ~15.5), it would have to drop to ~8 minutes, and that same constant governs the scheduled path where the problem does not exist. Heartbeating restores the intended meaning of "stale", which is that the run died, not that it was slow. `generate.sh` keeps today's behaviour exactly (it never starts the engine, renders only when it is already up, always returns 0) with a comment recording that the asymmetry is deliberate, and its lock release became compare-and-delete like the app's, since it does not heartbeat and an unconditional `rm` on an overrun would have handed a third caller permission to load the checkpoint alongside a live app render. Measured end to end: cold path 127 s wall clock from click to a 1080x1350 PNG (the server answers `/system_stats` within seconds, the checkpoint loads during sampling), warm path 120 s with no start attempted. 21 new tests (159 total).

**Files changed:** app/lib/comfy.ts, app/app/api/render/route.ts, app/lib/runLock.ts, app/components/OptionCard.tsx, engine/generate.sh, engine/render-visual.mjs, app/test/comfy.test.ts, app/test/runLock.test.ts, README.md
